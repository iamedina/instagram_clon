...
function Perfil() {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            // Si no hay userData, usar sesión de Supabase (Facebook)
            if (!userData) {
                const { data: { session } } = await supabase.auth.getSession();
                if (session?.user) {
                    userData = {
                        id: session.user.id,
                        username: session.user.user_metadata?.nickname || session.user.email,
                                                    ...
                                                    <div className="flex w-full items-center gap-x-25 pb-5 max-md:flex-col max-md:items-center">
                                                        ...
                                                        <div className="flex flex-col">
                                                            <div className="flex items-center gap-4 mb-2 flex-wrap justify-center  md:justify-start">
                                                                <div><h2 className="text-xl font-normal cursor-pointer ">{user.username}</h2></div>
