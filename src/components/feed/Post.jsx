...
function Post({ post }) {
    const { type, files } = post;
    const [idx, setIdx] = useState(0);


    const prev = () => setIdx(p => Math.max(0, p - 1));
    const next = () => setIdx(p => Math.min(files.length - 1, p + 1));
                                                        ...
                                                        {type === "video" ? (
                                                            ...
                                                            <div className="relative">
                                                                <div className="relative w-full max-h-[520px] overflow-hidden">
                                                                    <img src={files[idx].url} className="w-full h-full object-cover" />
                                                                </div>
                                                            ...
                                                            <img src={url} alt="´post imagen" className="object-cover w-[100%] h-[100%] top-0 user-select-none absolute  inset-0 border-0" />
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
